...
### Estoque/Promoção — desconto por valor (R$) além do %

Na barra de Promoção do Estoque, além do campo de **%**, há um campo de **valor (R$)** com máscara da moeda brasileira. Ao digitar o valor, o **% é preenchido automaticamente** (prévia calculada pelo preço médio dos produtos selecionados; com 1 produto ou preços iguais, é exato). No **Aplicar**, se houver valor em R$, o servidor converte o desconto em **% por produto** (cada um recebe o % equivalente ao seu próprio preço — exato mesmo com preços diferentes; limitado a 99%). Digitar o % manualmente limpa o campo de valor (modo %). Rota `/estoque/promocao` passou a aceitar `valor_desconto`.
...
### Nova venda — cadastrar cliente sem sair da venda

Ao informar o cliente na venda e ele não existir, o "cadastrar aqui" abre um **modal de cadastro** (nome, CPF, nascimento, telefones, endereço, UF, crediário e promoções). Ao **salvar**, o cliente é criado via AJAX (`/clientes/novo-rapido`, retorna JSON) e **já entra selecionado na venda** (habilitando o crediário se marcado) — sem recarregar a página nem perder a venda em andamento. Se já existir um cliente com o mesmo nome, ele é reaproveitado (não duplica). O modal tem **máscaras** de CPF, telefone e CEP (com **busca de endereço** pelo ViaCEP preenchendo logradouro/bairro/cidade/UF) e **capitaliza a inicial** de nome, logradouro, bairro, cidade e complemento (Title Case), evitando nomes começando em minúscula.
