...
def reset_usuarios():
    ...
    try:
        for cod, nome, senha, perfil in [
            ...
            perms = 'visao_geral,clientes,vendas,estoque,caixa,crediarios,despesas,usuarios,relatorios,dashboards'
            cur.execute("SELECT id FROM usuarios WHERE nome=%s OR codigo=%s",(nome,cod))
            u = cur.fetchone()
            if u: cur.execute("UPDATE usuarios SET codigo=%s,senha_hash=%s,perfil=%s,permissoes=%s,ativo=TRUE WHERE id=%s",(cod,h,perfil,perms,u['id']))
            else: cur.execute("INSERT INTO usuarios (codigo,nome,senha_hash,perfil,permissoes) VALUES (%s,%s,%s,%s,%s)",(cod,nome,h,perfil,perms))
        conn.commit()
        return """<div style='font-family:sans-serif;padding:40px;max-width:500px;margin:0 auto'>
            <h2>✅ Usuários resetados!</h2>
            <p style='margin:16px 0'><strong>Renan Barcellos</strong> → senha: <code>renan123</code></p>
            <p style='margin:16px 0'><strong>Carol Duarte</strong> → senha: <code>carol123</code></p>
            <br><a href='/' style='background:#1a1a2e;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none'>Ir para o login →</a></div>"""
    except Exception as e:
        conn.rollback(); return f"<pre>ERRO: {e}</pre>"
...
def visao_geral():
    ...
    cur.execute("SELECT COALESCE(SUM(CASE WHEN tipo='entrada' THEN valor ELSE -valor END),0) as v FROM caixa WHERE DATE_TRUNC('month',criado_em)=DATE_TRUNC('month',NOW())")
    saldo_caixa = float(cur.fetchone()['v'])

    # Movimentações recentes
    cur.execute("""SELECT v.id,v.codigo,v.criado_em,v.vendedora_nome,v.cliente_nome,
                   v.valor_total,v.forma_pagamento FROM vendas v ORDER BY v.criado_em DESC LIMIT 8""")
    ...
    ctx.update(fat=fat, fat_total=fat_total, val_estoque=val_estoque,
               ...
               saldo_caixa=saldo_caixa, movs=movs, estoque_baixo=estoque_baixo,
    ...
    if request.method == 'POST':
        ...
        try:
            cur.execute("INSERT INTO usuarios (codigo,nome,senha_hash,perfil,permissoes) VALUES (%s,%s,%s,%s,%s)",
                        (cod,nome,generate_password_hash(senha),perfil,perms))
...
def clientes():
    ...
    ctx = get_ctx()
    ctx.update(clientes=lista, next_id=f"C{n+1}")
...
def novo_cliente():
    nome = request.form.get('nome','').strip()
    if not nome: flash('Nome obrigatório.','erro'); return redirect(url_for('clientes'))
    conn = get_db(); cur = conn.cursor()
    cur.execute("SELECT id FROM clientes WHERE LOWER(TRIM(nome))=LOWER(TRIM(%s))",(nome,))
    if cur.fetchone():
        cur.close(); conn.close()
        flash(f'DUPLICADO_NOME||{nome}','erro'); return redirect(url_for('clientes'))
    cpf = request.form.get('cpf','').strip()
    if cpf:
        cur.execute("SELECT id FROM clientes WHERE cpf=%s",(cpf,))
        if cur.fetchone():
            cur.close(); conn.close()
            flash(f'DUPLICADO_CPF||{cpf}','erro'); return redirect(url_for('clientes'))
    cur.execute("SELECT COUNT(*) as t FROM clientes"); n = cur.fetchone()['t']
    cod = f"C{n+1}"
    try:
        cur.execute("""INSERT INTO clientes (codigo,nome,cpf,data_nascimento,telefone,telefone2,
            ...
            (cod,nome,cpf or None,request.form.get('data_nascimento') or None,
...
def novo_estoque():
    ...
    try:
        cur.execute("""INSERT INTO estoque (codigo,modelo,descricao,tamanho,quantidade,estoque_inicial,
            ...
            (cod, request.form.get('modelo','').strip(),
             request.form.get('descricao','').strip() or None,
             request.form.get('tamanho','').strip(),
             qtd, qtd, custo, markup, venda, margem))
    ...
    if request.method == 'POST':
        ...
        cur.execute("""UPDATE estoque SET modelo=%s,descricao=%s,tamanho=%s,quantidade=%s,
            ...
            (request.form.get('modelo','').strip(),
             request.form.get('descricao','').strip() or None,
             request.form.get('tamanho','').strip(), qtd, custo,
             float(request.form.get('markup',0) or 0), venda,
...
def vendas():
    ...
    try:
        ...
        meses = [{'mes_val':m['mes'].strftime('%Y-%m'),'mes_label':m['mes'].strftime('%B / %Y').capitalize()}
        ...
        return f"<pre style='padding:20px'>ERRO VENDAS:\n{e}\n\n{traceback.format_exc()}</pre>",500
