...
ABAS = [
    ('visao_geral','Visao Geral'),('clientes','Clientes'),('vendas','Vendas'),
    ('estoque','Estoque'),('caixa','Caixa'),('crediarios','Crediarios'),
    ('despesas','Despesas'),('usuarios','Usuarios'),('dashboards','Dashboards'),
]
...
def init_db():
    ...
    for m in ['Vestido','Calca','Blusa','Bolsa','Saia','Macacao','Conjunto','Short','Blazer','Kimono']:
...
def setup():
    try:
        init_db()
        return "<h2 style='font-family:sans-serif;padding:40px'>Banco OK! <a href='/'>Login</a></h2>"
    ...
    try:
        cur.execute("""INSERT INTO estoque (codigo,modelo,descricao,tamanho,quantidade,estoque_inicial,
            ...
            (f"P{n+1}", request.form.get('modelo','').strip(),
             ...
             request.form.get('tamanho','').strip(), qtd, qtd, custo,
