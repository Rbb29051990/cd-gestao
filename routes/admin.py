...
def versao():
    ...
    v139: Despesas — clicar na linha (categoria/descrição) abre o DETALHAMENTO completo: todas as parcelas com vencimento, valor, quais estão pagas e quando (forma e observação). No "Editar" dá para RENEGOCIAR valor e vencimento das parcelas ainda EM ABERTO (parcelas pagas não mudam); o total da despesa recalcula sozinho. Ao mudar a DATA de uma parcela, as seguintes em aberto viram +30 dias em cascata; ao mudar um VALOR, a diferença redistribui igualmente entre as outras parcelas em aberto (total constante). Saíram os botões de editar/excluir da linha (excluir agora fica dentro do detalhamento, N1). ✅<br>
